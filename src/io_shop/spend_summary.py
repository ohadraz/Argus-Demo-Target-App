...
def average_spend_per_item(account: Account) -> int:
    ...
    even where the totals the query returned have drifted from it. Summed in one
    pass: the running totals of every prefix but the last are worth nothing to
    anybody, and computing them turns the most-visited page in the shop into
    work that grows with the square of a shopper's history.
    """
