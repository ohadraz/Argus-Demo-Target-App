...
def average_spend_per_item_this_month(account: Account) -> int:
    ...
    the exceptional one - early in a month it is most of them - so the divisor
    here is empty far more often than the lifetime one's ever is. There is no
    average of nothing to report, and nothing spent, so the figure is zero.
    Dividing anyway would turn a quiet month into a failed page.
    """
    bought_this_month = [
