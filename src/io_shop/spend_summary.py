...
def average_spend_per_item_this_month(account: Account) -> int:
    ...
    broken one: most accounts are idle on most days. There is nothing to
    average over, and the figure for that is zero - so the divisor is only used
    once there is something in it. Raising here would fail the whole page for a
    shopper whose only fault is not having shopped yet this month.
    """
    bought_this_month = [
