...
def test_the_monthly_average_is_zero_for_a_shopper_who_has_not_bought_this_month() -> None:
    # The incident: a shopper with history but an empty month is ordinary, not
    # impossible, and used to divide by zero the moment the flag came on.
    account = an_account_with_no_purchases_this_month(1000, 2000, 3000)

    assert average_spend_per_item_this_month(account) == 0


def test_the_monthly_average_is_zero_for_a_shopper_who_never_bought_anything() -> None:
...
def test_the_page_renders_the_monthly_summary_for_an_empty_month() -> None:
    # With the flag on, the same shopper the rollout broke now renders a figure.
    account = an_account_with_no_purchases_this_month(1000, 3000)
