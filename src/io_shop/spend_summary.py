...
Every figure here is on the account page's fallback path - the one taken
whenever the summary cache holds nothing or cannot be reached at all. That
makes the cost of these functions the shop's own latency on the day the cache
goes away, which is why each of them walks the history once and no more.
"""
...
def average_spend_per_item(account: Account) -> int:
    ...
    this runs on every request the cache could not serve, and a figure that
    re-summed the history once per purchase turned a lost cache into a
    slowdown of the whole shop.
    """
