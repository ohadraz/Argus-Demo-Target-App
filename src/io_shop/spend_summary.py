...
def average_spend_per_item(account: Account) -> int:
    ...
    even where the totals the query returned have drifted from it. The
    purchases are added up once: re-summing the history once per purchase gives
    the same answer at a cost that grows with the square of it, which an
    account page pays for the shoppers who have bought the most.
    """
