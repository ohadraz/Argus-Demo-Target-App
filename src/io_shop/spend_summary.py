...
def average_spend_per_item(account: Account) -> int:
    ...
    the purchases is all that takes: the running total each purchase would have
    reached is not needed, only the last of them, and rebuilding it per purchase
    costs the page O(n^2) on a shopper with any real history.
    """
